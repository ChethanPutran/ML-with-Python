...
std::vector<Lidar_Measurement> Lidar_Sensor::generarate_lidar_measurement(double sensor_x, double sensor_y, double sensor_yaw, const Beacon_Map &map)
{
    ...
    for (const Beacon_Data &beacon : map.get_beacons())
    {
        double delta_x = beacon.x - sensor_x;
        double delta_y = beacon.y - sensor_y;
        double theta = wrap_angle(atan2(delta_y, delta_x) - sensor_yaw);
        ...
        if (beacon_range < m_max_range)
        {
            ...
            beacon_meas.theta = wrap_angle(theta + lidar_theta_dis(this->m_rand_gen));
